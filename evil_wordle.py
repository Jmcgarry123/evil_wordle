    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for pos, letter in enumerate(guessed_word):
            ...
            if feedback == CORRECT_COLOR:
                ...
                if color_current not in (CORRECT_COLOR, WRONG_SPOT_COLOR)   :
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        for color in self.feedback_colors:
            if color in self.COLOR_DIFFICULTY:
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(5):
        if (feedback[i] == NOT_IN_WORD_COLOR
            and guessed_word[i] in secret_count and secret_count[guessed_word[i]] > 0):
